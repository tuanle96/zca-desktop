...
1. Run the helper with a title and optional flags:
   ...
   ```bash
   node .kiro/skills/create-story/create-story.mjs "Feature title" --classification=normal --hours=2
   ```

   If the repo has no obvious test command, pass concrete verification up front:

   ```bash
   node .kiro/skills/create-story/create-story.mjs "Feature title" --verify-command="npm test -- feature-name"
   ```

   For high-risk work, declare the implementation layer scope up front:

   ```bash
   node .kiro/skills/create-story/create-story.mjs "Feature title" --classification=high-risk --hours=4 --layers=service,runtime
   ```
