 ...
 * the device must be re-linked — so we stop retrying and drop back to the gate
 * instead of looping on 401/403 or leaking the raw error onto the login screen. */
function isCloudAuthError(value: unknown): boolean {
...
function defaultDeviceName(): string {
    if (typeof navigator !== "undefined") {
        ...
        if (ua.includes("Mac")) return "Máy Mac";
...
class SessionStore {
    ...
    /** Per-account set of message ids already observed, so a re-hydrate only
     * notifies for genuinely new incoming messages. Seeded (without notifying)
     * on an account's first hydrate so existing history never pops a burst. */
    ...
    async restoreCloudDevice(baseUrl?: string): Promise<boolean> {
        ...
        // Restore failed. Drop back to the gate quietly. If the token is dead
        // (revoked, or the server DB was reset), purge it so we don't loop on
        // 401 every launch and so the login form shows clean.
    ...
    /**
     * Connect using a cloud device token. Validates the token by listing
     * accounts BEFORE opening the realtime stream, so a dead token fails fast
     * instead of spinning in a 401 reconnect loop. Returns a discriminated
     * result so callers can distinguish "no accounts yet" from "token rejected".
     */
    async connectCloud(baseUrl: string, deviceToken: string): Promise<"connected" | "auth-error" | "failed"> {
        try {
            ...
            if (accounts.length === 0) {
                // Token is valid but no Zalo account linked yet. Open realtime so
                // a subsequent QR link goes live, but stay on the gate.
    ...
    /** Reset in-memory cloud connection state without touching stored secrets. */
    ...
    /**
     * Complete device linking from a magic-link deep link (ADR-0009): verify the
    ...
    private activate(accountId: string) {
        ...
        notifications.setActiveThread(target.activeThreadId);
        this.view = "chats";
    ...
    async hydrateCloudHistory(accountId?: string) {
        ...
        // Notification baseline (ADR-0010): the first hydrate for an account
        // seeds the seen-set silently so existing history never pops a burst;
        // later hydrates collect only genuinely-new incoming messages to notify.
        let seen = this.seenMessageIds.get(id);
        const firstHydrate = seen === undefined;
        if (!seen) {
            seen = new Set<string>();
            this.seenMessageIds.set(id, seen);
        }
        const newIncoming: { title: string; body: string; threadId: string }[] = [];
        ...
        for (const row of rows) {
            ...
            // Detect new incoming messages for OS notifications (ADR-0010). Only
            // run when this row's fetch succeeded (else mapped is empty/stale).
            // On the first hydrate we only seed the seen-set; afterward each
            // not-yet-seen incoming (non-deleted) message becomes a candidate.
            if (!messageFetchFailed) {
                const convoTitle = row.title || row.threadId;
                for (const m of mapped) {
                    if (seen.has(m.id)) continue;
                    seen.add(m.id);
                    if (firstHydrate) continue;
                    if (m.outgoing || m.deleted) continue;
                    newIncoming.push({
                        title: m.authorName || convoTitle,
                        body: messageSnippet(m.body, m.sticker, m.link, m.deleted),
                        threadId: row.threadId,
                    });
                }
            }
        ...
        // Fire OS notifications for the newly-observed incoming messages. The
        // store applies the trigger policy (enabled + unfocused/non-active
        // thread) and is fully non-secret (display title + snippet only). Cap
        // the burst so a long catch-up hydrate can't flood the OS center.
        if (newIncoming.length) {
            for (const item of newIncoming.slice(-5)) void notifications.notify(item);
        }
    }

    /** Load the account's friends/contacts for the Danh bạ pane (ADR-0008).
     * In cloud mode this calls the hosted contacts directory endpoint; the
     * result is cached per-account bucket so it is fetched once per session and
     * only refreshed on explicit user action. */
    async loadContacts(accountId?: string) {
        const id = accountId ?? this.activeAccountId;
        if (!id) return;
        if (!this.cloudBaseUrl || !this.cloudDeviceToken) return;
        await this.run(async () => {
            const contacts = (await listCloudContacts(
                this.cloudBaseUrl!,
    ...
    selectThread(threadId: string) {
        ...
        notifications.setActiveThread(threadId);
            ...
            this.unlistenCloud = await listen<CloudRealtimeEvent>(CLOUD_EVENT, (event) => {
                ...
                if (payload.type === "error") {
                    if (payload.status === 401 || payload.status === 403) {
                        ...
                        void this.handleCloudAuthFailure();
                        return;
                    }
    ...
    /** The cloud device token was rejected (revoked, or the server was reset).
     * Stop retrying, purge the dead token, and drop back to the login gate
     * without surfacing a scary error — the user just needs to re-link. */
    ...
    openThread(threadId: string, title?: string) {
        ...
        notifications.setActiveThread(id);
    }
