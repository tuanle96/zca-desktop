...
/// Fetch the account's friends and map them into non-secret [`ContactView`]
/// DTOs (ADR-0008). Confined to the `zalo_host` boundary so higher layers never
/// see `zca-rust`'s `User`. Returns contacts sorted by display name
/// (case-insensitive).
pub async fn list_contacts(api: &API) -> ZaloResult<Vec<crate::models::ContactView>> {
    ...
    let mut contacts: Vec<crate::models::ContactView> = users
        .into_iter()
        .map(|u| crate::models::ContactView {
